...
const Actions = ({ original }: { original: ProductVariant }) => {
  ...
  const onDelete = async () => {
    const shouldDelete = await dialog({
      heading: "Kustuta nimiväärtus",
      text: "Kas soovite kindlasti selle nimiväärtuse kustutada?",
    })
    ...
    if (shouldDelete) {
      mutateAsync(original.id, {
        onSuccess: () => {
          notification(
            "Nimiväärtus kustutatud",
            "Nimiväärtus on edukalt kustutatud",
            "success"
          )
        },
        onError: (error) => {
