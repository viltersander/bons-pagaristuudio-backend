...
function CustomerGroupsPlaceholder() {
  return (
    <div className="center flex h-full min-h-[756px] items-center justify-center">
      <span className="text-xs text-gray-400">Kliendigruppe pole veel</span>
    </div>
  )
}
...
function CustomerGroupsTableRow(props: CustomerGroupsTableRowProps) {
  ...
  // Hook for deleting a customer group
  const { isLoading, error, data, mutate } = useAdminDeleteCustomerGroup(row.original.id);
