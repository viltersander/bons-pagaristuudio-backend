    ...
    >
      ...
      {!rows.length && !locationsLoading && !stock_locations?.length && (
        <div className="text-grey-50 w-full py-8 text-center">
          Teil pole lao asukohti. Lisage üks, et näha laoseisu.
        </div>
      )}
    </TableContainer>
  )
    ...
    updateLocationLevelForInventoryItem(
      ...
      },
      {
        onSuccess: () => {
          notification(
            "Õnnestus",
            "Laoüksuse värskendamine õnnestus",
            "success"
          )
          handleClose()
        },
    ...
    <Modal handleClose={handleClose}>
      <Modal.Body>
        <Modal.Header handleClose={handleClose}>
          <h1 className="inter-large-semibold">Reguleerige saadavust</h1>
        </Modal.Header>
        <Modal.Content>
          {isLoading ? (
            <Spinner />
