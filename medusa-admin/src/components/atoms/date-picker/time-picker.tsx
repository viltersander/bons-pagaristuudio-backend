...
const TimePicker: React.FC<DateTimePickerProps> = ({
  ...
  const [selectedMinute, setSelectedMinute] = useState<number | null>(
    moment(date).tz("Europe/Tallinn").get("minutes")
  );
  const [selectedHour, setSelectedHour] = useState<number | null>(
    moment(date).tz("Europe/Tallinn").get("hours")
  );

  useEffect(() => {
    setSelectedMinute(moment(date).tz("Europe/Tallinn").get("minutes"));
    setSelectedHour(moment(date).tz("Europe/Tallinn").get("hours"));
  }, [date]);

  useEffect(() => {
    if (date && !isNil(selectedHour) && !isNil(selectedMinute)) {
      const newDate = moment(date).tz("Europe/Tallinn").startOf("day").toDate();
  ...
  const formatTime = (date: Date | null) => {
    ...
    return moment(date).tz("Europe/Tallinn").format("HH:mm");
  };
