...
const NewGiftCard = ({ onClose }: NewGiftCardProps) => {
  ...
  const onSubmit = async (data: NewGiftCardFormData) => {
    ...
    if (!trimmedName) {
      notification("Viga", "Sisestage kinkekaardi nimi", "error")
      focusByName("name")
      return
    }

    if (!data.denominations?.length) {
      notification("Viga", "Palun lisage vähemalt üks nimiväärtus", "error")
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          notification("Õnnestus", "Kinkekaardi loomine õnnestus", "success")
          refetch()
      ...
      <form onSubmit={handleSubmit(onSubmit, (err) => console.log(err))}>
        <Modal.Body>
          <Modal.Header handleClose={onClose}>
            <div>
              <h1 className="inter-xlarge-semibold">Loo kinkekaart</h1>
            </div>
          </Modal.Header>
          <Modal.Content>
            <div className="mb-base">
              <h3 className="inter-base-semibold">Kinkekaardi üksikasjad</h3>
            </div>
            <div className="gap-y-base flex flex-col">
              <InputField
                label={"Nimi"}
                required
                placeholder="Parim kinkekaart"
                {...register("title", { required: true })}
              />
              <TextArea
                label="Kirjeldus"
                placeholder="Kõigi aegade parim kinkekaart"
                {...register("description")}
              />
            </div>
            <div className="mt-xlarge">
              <h3 className="inter-base-semibold">Pisipilt</h3>
            ...
            <div className="mt-xlarge">
              <h3 className="inter-base-semibold mb-base">
                Nimiväärtused<span className="text-rose-50">*</span>
              </h3>
