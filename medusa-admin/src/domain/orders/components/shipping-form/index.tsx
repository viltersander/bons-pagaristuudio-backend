  ...
  return (
    <div className="gap-y-base flex flex-col">
      <div className="flex flex-col">
        ...
        Kohaletoimetame {isReturn ? "tagastatud" : "asendatud"} esemed
        </h2>
      ...
      <Controller
        control={control}
        name={path("option")}
        rules={{ required: required ? `Kohaletoimetamise viis on nõutav` : false }}
        render={({ field: { value, onChange, onBlur, ref, name } }) => {
          return (
            <NextSelect
              ref={ref}
              placeholder="Valige saatmisviis"
              label="Saatmisviis"
              name={name}
...
const ShippingFormHelpText = ({
  ...
  const text = useMemo(() => {
    if (isClaim && isReturn) {
      return "Kliendi esitatud kaupade tagastamine on tasuta."
    }

    if (!isReturn) {
      return "Asenduskaupade saatmine on tasuta."
    }
