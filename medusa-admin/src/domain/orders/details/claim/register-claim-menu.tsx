  ...
  const onCancel = async () => {
    ...
    if (isDirty) {
      shouldClose = await dialog({
        heading: "Kas soovite kindlasti sulgeda?",
        text: "Teil on salvestamata muudatusi. Kas soovite kindlasti sulgeda?",
      })
    }
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    if (type === "replace" && !data.replacement_shipping.option) {
      setError(
        ...
        {
          type: "manual",
          message: "Vajalik on asenduskaupade saatmisviis",
        },
        { shouldFocus: true }
      )
      return
    ...
    mutate(
      ...
      },
      {
        onSuccess: () => {
          notification(
            "Nõude loomine õnnestus",
            `Nõue tellimuse saamiseks #${order.display_id} loodi edukalt`,
            "success"
          )
          handleClose()
        },
        onError: (err) => {
          notification("Nõude loomisel tekkis viga", getErrorMessage(err), "error")
        },
      }
    )
  })
    ...
    >
      <Modal.Body>
        <Modal.Header handleClose={onCancel}>
          <h1 className="inter-xlarge-semibold">Loo nõue</h1>
        </Modal.Header>
